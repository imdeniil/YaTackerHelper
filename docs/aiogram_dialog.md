# Особенности работы с aiogram-dialog

Документация по специфическим особенностям библиотеки aiogram-dialog, обнаруженным в процессе разработки.

---

## Проблема: Progress виджет и редактирование сообщений

### ❌ Симптомы

При использовании Progress виджета в фоновой задаче с переходом между окнами через `switch_to()`:

1. **Новое сообщение вместо редактирования**
   - При `switch_to()` между окнами Telegram создаёт новое сообщение
   - Progress обновляется, но каждое обновление = новое сообщение
   - Чат засоряется множеством сообщений с прогрессом

2. **BgManager не помогает**
   - Использование `bg.start()` с разными `stack_id` приводит к ошибкам:
     - `IncorrectBackgroundError` - нельзя использовать `manager.dialog_data`
     - `UnknownIntent` - контекст не сохраняется
   - UI возвращается на предыдущее окно вместо показа прогресса

### 🔍 Причина

**Ключевая проблема:** `switch_to()` между **разными окнами** всегда создаёт новое сообщение.

```python
# ❌ НЕПРАВИЛЬНО - создаст новое сообщение
await manager.switch_to(CloneProject.progress)  # переход на другое окно
# Telegram отправит новое сообщение с окном progress
```

aiogram-dialog **не может редактировать сообщение** при переходе между окнами, потому что это разные State с разным содержимым.

---

## ✅ Решение: Динамическое окно (Подход 9)

### Концепция

**Не переключаться между окнами!** Остаться на одном окне и менять его содержимое через условия `when`.

### Архитектура

Одно окно с **тремя динамическими состояниями**:

```python
Window(
    # === СОСТОЯНИЕ 1: Подтверждение ===
    Const("Подтверждение", when=~F["is_running"] & ~F["result"]),
    Button(..., when=~F["is_running"] & ~F["result"]),

    # === СОСТОЯНИЕ 2: Выполнение (Progress) ===
    Const("Выполнение...", when=F["is_running"]),
    Progress("progress", 10, when=F["is_running"]),

    # === СОСТОЯНИЕ 3: Результат ===
    Const("Завершено!", when=~F["is_running"] & F["result"]),
    Button(..., when=~F["is_running"] & F["result"]),

    state=MyState.main,  # ← ОДНО окно для всех состояний
    getter=get_dynamic_data,
)
```

### Механизм работы

1. **Handler запускает фоновую задачу**
   ```python
   async def on_start_task(callback: CallbackQuery, button: Button, manager: DialogManager):
       # Устанавливаем флаг выполнения
       manager.dialog_data["is_running"] = True
       manager.dialog_data["progress"] = 0

       # ❌ НЕ делаем switch_to! Остаёмся на том же окне
       # await manager.switch_to(MyState.progress)  # ← УБРАЛИ

       # Создаём BgManager
       bg = manager.bg()

       # Запускаем фоновую задачу
       asyncio.create_task(background_task(bg))
   ```

2. **Фоновая задача обновляет данные**
   ```python
   async def background_task(manager: DialogManager):
       for i in range(100):
           # Обновляем через manager.update()
           await manager.update({
               "is_running": True,
               "progress": i,
           })
           await asyncio.sleep(0.1)

       # Завершено
       await manager.update({
           "is_running": False,
           "result": "Успешно",
       })
   ```

3. **Окно само перерисовывается**
   - При `is_running = True` показывается Progress
   - При `is_running = False` и `result` есть → показывается результат
   - **Сообщение редактируется**, а не создаётся новое!

### Getter

```python
async def get_dynamic_data(dialog_manager: DialogManager, **kwargs):
    """Getter возвращает данные для всех состояний."""
    return {
        # Для подтверждения
        "task_name": dialog_manager.dialog_data.get("task_name", ""),

        # Для прогресса
        "is_running": dialog_manager.dialog_data.get("is_running", False),
        "progress": dialog_manager.dialog_data.get("progress", 0),

        # Для результата
        "result": dialog_manager.dialog_data.get("result"),
        "error": dialog_manager.dialog_data.get("error"),
    }
```

---

## Реализация в проекте

### Файл: `bot/dialogs/clone_project.py`

**Окно `confirm_clone`** объединяет три состояния:

```python
Window(
    # === СОСТОЯНИЕ 1: Подтверждение (is_cloning=False, result=None) ===
    Format("📁 Исходный проект: <b>{project_name}</b>", when=~F["is_cloning"] & ~F["result"]),
    Format("📝 Новый проект: <b>{new_name}</b>", when=~F["is_cloning"] & ~F["result"]),
    Format("📮 Очередь: <b>{queue}</b>\n", when=~F["is_cloning"] & ~F["result"]),
    Const("⚠️ Начать клонирование?", when=~F["is_cloning"] & ~F["result"]),
    Row(
        Button(Const("🚀 Начать"), id="start_clone", on_click=on_start_clone,
               when=~F["is_cloning"] & ~F["result"]),
        Back(Const("◀️ Назад"), when=~F["is_cloning"] & ~F["result"]),
    ),

    # === СОСТОЯНИЕ 2: Клонирование (is_cloning=True) ===
    Format("\n{phase}\n", when=F["is_cloning"]),
    Progress("progress", 10, when=F["is_cloning"]),

    # === СОСТОЯНИЕ 3: Результат (is_cloning=False, result есть) ===
    Format("📁 Проект: <b>{new_project_name}</b>", when=~F["is_cloning"] & F["result"]),
    Format("📋 Создано задач: <b>{created_count}</b>\n", when=~F["is_cloning"] & F["result"]),
    Row(
        Url(Const("🔗 Открыть проект"), Format("{project_url}"),
            when=~F["is_cloning"] & F["result"]),
    ),
    Row(
        Cancel(Const("🏠 Главное меню"), when=~F["is_cloning"] & F["result"]),
    ),

    # Предотвращаем сброс во время выполнения
    MessageInput(on_message_during_clone),

    state=CloneProject.confirm_clone,  # ← ОДНО окно
    getter=get_final_confirm_data,
)
```

**Handler запуска:**

```python
async def on_start_clone(callback: CallbackQuery, button: Button, manager: DialogManager):
    """Запуск процесса клонирования (подход 9: динамическое окно)."""
    project_id = manager.dialog_data.get("project_id")
    new_name = manager.dialog_data.get("new_name")
    queue = manager.dialog_data.get("queue")

    # Инициализируем данные прогресса и устанавливаем флаг клонирования
    manager.dialog_data["is_cloning"] = True
    manager.dialog_data["progress"] = 0
    manager.dialog_data["phase"] = "Инициализация..."

    # ❌ НЕ делаем switch_to! Остаемся на confirm_clone
    # await manager.switch_to(CloneProject.progress)  # ← УБРАЛИ

    # Создаем BgManager для фоновой задачи
    bg = manager.bg()

    # Запуск фоновой задачи с BgManager
    asyncio.create_task(
        clone_project_background_with_manager(
            manager=bg,
            project_id=project_id,
            new_name=new_name,
            queue=queue
        )
    )
```

**Фоновая задача:**

```python
async def clone_project_background_with_manager(
    manager: DialogManager,
    project_id: str,
    new_name: str,
    queue: str
):
    """Фоновая задача клонирования проекта с использованием BgManager."""
    try:
        async with TrackerClient() as tracker:
            cloner = ProjectCloner(tracker)

            # Callback для обновления прогресса
            async def progress_update(value: float):
                total_progress = value * 0.5  # 0-50%
                phase = "📁 Получение проекта..." if value <= 5 else "🔄 Получение задач..."

                # Обновляем через manager.update()
                await manager.update({
                    "is_cloning": True,
                    "progress": int(total_progress),
                    "phase": phase,
                })

            cloner.set_progress_callback(progress_update)

            # Получение данных (0-50%)
            project_data = await cloner.fetch_project_data(project_id)

            # Клонирование (50-100%)
            async def clone_progress_update(value: float):
                total_progress = 50 + value * 0.5
                phase = "🚀 Создание задач..."

                await manager.update({
                    "is_cloning": True,
                    "progress": int(total_progress),
                    "phase": phase,
                })

            cloner.set_progress_callback(clone_progress_update)
            result = await cloner.clone_project(
                project_data=project_data,
                new_project_name=new_name,
                target_queue=queue
            )

            # Завершено - показываем результат
            await manager.update({
                "is_cloning": False,
                "result": result.success,
                "new_project_name": result.new_project_name,
                "new_project_short_id": result.new_project_short_id,
                "created_count": len(result.new_issues_mapping),
                "project_url": f"https://tracker.yandex.ru/pages/projects/{result.new_project_short_id}",
            })

    except Exception as e:
        # Ошибка
        await manager.update({
            "is_cloning": False,
            "result": False,
            "error": str(e),
        })
```

---

## Преимущества подхода

### ✅ Что улучшилось

1. **Редактирование сообщения**
   - Сообщение редактируется вместо создания нового
   - Плавное обновление UI
   - Нет лишних сообщений в чате

2. **Использование BgManager**
   - Не нужен прямой Bot API (`bot.edit_message_text()`)
   - `manager.update()` вместо ручного редактирования
   - Меньше кода, проще поддержка

3. **Progress виджет**
   - Стандартный виджет вместо ручного рендеринга
   - Автоматический progress bar
   - Встроенное отображение процентов

4. **Правильная архитектура**
   - Соответствует идеологии aiogram-dialog
   - Использование `when` условий вместо множества окон
   - Единый источник данных в getter

### ⚠️ Компромиссы

1. **Сложность окна**
   - Много условных виджетов с `when`
   - Может быть сложнее читать код

2. **Getter возвращает больше данных**
   - Нужно возвращать данные для всех состояний
   - Больше полей в словаре

---

## Альтернативные подходы (НЕ работают)

### ❌ Подход 1-8: switch_to() между окнами

Все подходы с использованием `switch_to()` между `confirm` и `progress` окнами создают новое сообщение.

**Протестировано:**
- Подход 1: `switch_to + bg` без `stack_id`
- Подход 2: `bg.start(RESET_STACK)`
- Подход 3: `bg` с отдельным `stack_id`
- Подход 4: `switch_to(EDIT) + bg` с задержкой
- Подход 5: `show_mode` в getter
- Подход 6: `done() -> bg.start()`
- Подход 7: `update()` вместо `switch_to` (но на неправильном окне)
- Подход 8: явная установка `show_mode` перед `bg`

**Результат:** Все создают новое сообщение или вызывают ошибки.

### ❌ Прямое использование Bot API

Можно использовать `bot.edit_message_text()` напрямую, но это:
- Обход архитектуры aiogram-dialog
- Ручное форматирование UI
- Больше кода
- Хуже поддержка

**Вывод:** Подход 9 (динамическое окно) - единственное правильное решение.

---

## Общие рекомендации

### Когда использовать динамическое окно

✅ **Используй**, если:
- Нужно отображать прогресс в real-time
- Задача выполняется в фоновом режиме
- Требуется редактирование сообщения (а не создание нового)
- Есть логическая последовательность: подтверждение → выполнение → результат

❌ **НЕ используй**, если:
- Окна логически независимы (разные секции меню)
- Нет фоновой задачи
- Не требуется real-time обновление

### Чеклист для динамического окна

1. ☑️ Одно окно для всех состояний
2. ☑️ Виджеты с условиями `when=F[...]`
3. ☑️ Getter возвращает данные для всех состояний
4. ☑️ Handler НЕ делает `switch_to()`
5. ☑️ Фоновая задача использует `manager.update()`
6. ☑️ Флаг состояния (`is_running`, `result`) для переключения
7. ☑️ `MessageInput` для предотвращения сброса

---

## Отображение ошибок валидации в MessageInput

### ❌ Проблема

При валидации пользовательского ввода в `MessageInput` хочется показать ошибку в том же диалоговом окне, а не отправлять новое сообщение.

**Неправильный подход:**
```python
async def on_amount_input(message: Message, widget: MessageInput, manager: DialogManager):
    amount = message.text.strip()

    try:
        amount_float = float(amount)
        if amount_float <= 0:
            raise ValueError()
    except ValueError:
        # ❌ НЕПРАВИЛЬНО - создаёт новое сообщение
        await message.answer("❌ Некорректная сумма. Введите число больше 0")
        return
```

**Результат:** При ошибке валидации отправляется новое сообщение, чат засоряется.

### ✅ Решение: switch_to с ShowMode.EDIT

Чтобы показать ошибку **в том же окне**, нужно:
1. Сохранить ошибку в `dialog_data`
2. Установить `show_mode = ShowMode.EDIT`
3. Переключиться **на то же состояние** с помощью `switch_to`

```python
async def on_amount_input(message: Message, widget: MessageInput, manager: DialogManager):
    """Обработчик ввода суммы"""
    if not message.text:
        # Сохраняем ошибку в dialog_data
        manager.dialog_data["error"] = "❌ Пожалуйста, отправьте текстовое сообщение"
        # Устанавливаем режим редактирования
        manager.show_mode = ShowMode.EDIT
        # Переключаемся на ТО ЖЕ окно
        await manager.switch_to(PaymentRequestCreation.enter_amount)
        return

    amount = message.text.strip()

    try:
        amount_float = float(amount.replace(",", ".").replace(" ", ""))
        if amount_float <= 0:
            raise ValueError("Amount must be positive")
    except ValueError:
        # То же самое при ошибке валидации
        manager.dialog_data["error"] = "❌ Некорректная сумма. Введите число больше 0"
        manager.show_mode = ShowMode.EDIT
        await manager.switch_to(PaymentRequestCreation.enter_amount)
        return

    # Успешная валидация - очищаем ошибку и переходим дальше
    manager.dialog_data.pop("error", None)
    manager.dialog_data["amount"] = amount
    manager.show_mode = ShowMode.EDIT
    await manager.switch_to(PaymentRequestCreation.enter_comment)
```

### Настройка Window для отображения ошибок

В `Window` нужно добавить виджет для отображения ошибки с условием `when="error"`:

```python
amount_window = Window(
    Format("💰 <b>Создание запроса на оплату</b>\n\n"
           "Название: <i>{title}</i>\n\n"
           "Шаг 2 из 4: Введите сумму в рублях\n\n"
           "Например: <i>5000</i> или <i>5000.50</i>"),

    # Виджет для отображения ошибки (показывается только когда есть error)
    Format("\n{error}\n", when="error"),

    Cancel(Const("❌ Отмена")),
    MessageInput(on_amount_input),
    state=PaymentRequestCreation.enter_amount,
    getter=get_amount_data,
)
```

### Getter должен возвращать error

```python
async def get_amount_data(dialog_manager: DialogManager, **kwargs) -> dict[str, Any]:
    """Получает данные для окна ввода суммы"""
    title = dialog_manager.dialog_data.get("title", "")
    return {
        "title": title,
        "error": dialog_manager.dialog_data.get("error"),  # ← Возвращаем ошибку
    }
```

### Почему это работает?

1. **`show_mode = ShowMode.EDIT`**
   - Говорит aiogram-dialog редактировать сообщение, а не создавать новое

2. **`switch_to(CURRENT_STATE)`**
   - Переключение на то же состояние заставляет вызвать getter
   - Getter возвращает обновлённые данные (включая `error`)
   - aiogram-dialog обновляет окно с новым содержимым

3. **Виджет `Format("\n{error}\n", when="error")`**
   - Показывается только когда `error` есть в данных
   - Отображает текст ошибки прямо в диалоговом окне

### ❌ Что НЕ работает

**Вариант 1: Просто return без switch_to**
```python
manager.dialog_data["error"] = "❌ Ошибка"
return  # ❌ Окно не обновится
```
Окно не обновится, ошибка не появится.

**Вариант 2: manager.update()**
```python
manager.dialog_data["error"] = "❌ Ошибка"
await manager.update({})  # ❌ Может создать дубликат сообщения
return
```
`MessageInput` сам обновляет окно после handler'а, вызов `update()` создаст двойное обновление.

**Вариант 3: Новое сообщение через message.answer()**
```python
await message.answer("❌ Ошибка")  # ❌ Создаёт новое сообщение
```
Создаёт отдельное сообщение, засоряет чат.

### Реализация в проекте

- **Handlers:** `bot/dialogs/payment_request.py:105-182`
  - `on_title_input` (строка 105)
  - `on_amount_input` (строка 134)
  - `on_comment_input` (строка 162)

- **Windows:** `bot/dialogs/payment_request.py:323-364`
  - Каждое окно с вводом содержит `Format("\n{error}\n", when="error")`

---

## Обработка устаревших диалоговых окон (UnknownIntent)

### ❌ Проблема

При перезапуске бота или истечении контекста диалога пользователь может нажать на кнопки в старом диалоговом окне. aiogram-dialog выбрасывает исключение `UnknownIntent`:

```
aiogram_dialog.api.exceptions.UnknownIntent: Context not found for intent id: 1ca1f9
```

**Причина:**
- Контекст диалога хранится в MemoryStorage и теряется при перезапуске
- intent_id не существует после потери контекста
- Ошибка возникает в middleware aiogram-dialog, ДО обработки update

### ✅ Решение: Error Handler

**ВАЖНО:** Middleware НЕ поможет, так как ошибка возникает в middleware aiogram-dialog, который выполняется раньше.

Правильное решение - **глобальный error handler**:

```python
# bot/middlewares/unknown_intent.py
from aiogram import Router
from aiogram.types import ErrorEvent, CallbackQuery
from aiogram_dialog.api.exceptions import UnknownIntent

unknown_intent_router = Router()

@unknown_intent_router.error()
async def handle_unknown_intent(event: ErrorEvent):
    """Error handler для UnknownIntent исключений"""

    # Проверяем что это UnknownIntent ошибка
    if not isinstance(event.exception, UnknownIntent):
        return

    # Обрабатываем только callback query
    if not isinstance(event.update.callback_query, CallbackQuery):
        return

    callback = event.update.callback_query

    try:
        # Удаляем старое сообщение с устаревшими кнопками
        await callback.message.delete()
    except Exception:
        pass

    # Отправляем новое сообщение
    await callback.message.answer(
        "⚠️ <b>Это окно устарело</b>\n\n"
        "Используйте команду /start для возврата в главное меню."
    )

    # Отвечаем на callback чтобы убрать "часики" в Telegram
    await callback.answer("Окно устарело, используйте /start")

    # Возвращаем True чтобы отметить ошибку как обработанную
    return True
```

### Регистрация в main.py

```python
from bot.middlewares import unknown_intent_router

async def main():
    # ...

    # Регистрация роутеров
    dp.include_router(unknown_intent_router)  # Error handler должен быть первым
    dp.include_router(commands_router)
    # ... остальные роутеры
```

### Почему именно Error Handler?

1. **Глобальная обработка**
   - Ловит исключения на уровне диспетчера
   - Работает даже если ошибка в middleware aiogram-dialog
   - Не зависит от порядка middleware

2. **Приоритет**
   - Error handlers обрабатываются после всех middleware
   - Могут перехватить любое необработанное исключение

3. **Возвращает True**
   - Помечает ошибку как обработанную
   - Предотвращает появление в логах как необработанное исключение

### ❌ Почему Middleware НЕ работает

```python
# ❌ НЕПРАВИЛЬНО - middleware выполняется ПОСЛЕ aiogram-dialog middleware
class UnknownIntentMiddleware(BaseMiddleware):
    async def __call__(self, handler, event, data):
        try:
            return await handler(event, data)
        except UnknownIntent:  # ← Не перехватит, ошибка раньше
            # ...
```

**Проблема:**
- aiogram-dialog middleware выполняется раньше
- Исключение выбрасывается ДО вашего middleware
- Ошибка не перехватывается

### Реализация в проекте

- **Error handler:** `bot/middlewares/unknown_intent.py`
- **Регистрация:** `main.py:129` (первый роутер)
- **Экспорт:** `bot/middlewares/__init__.py`

---

## Источники

- [aiogram-dialog Documentation](https://aiogram-dialog.readthedocs.io/)
- [Progress Widget](https://aiogram-dialog.readthedocs.io/en/stable/widgets/text/progress/)
- [Background Manager](https://aiogram-dialog.readthedocs.io/en/stable/faq.html)
- Тестовый диалог: `bot/dialogs/test_progress_dialog.py`
- Реализация: `bot/dialogs/clone_project.py:737-789`

---

## История изменений

### 2025-12-30
- Добавлен раздел "Отображение ошибок валидации в MessageInput"
- Описано решение через switch_to с ShowMode.EDIT
- Добавлен раздел "Обработка устаревших диалоговых окон (UnknownIntent)"
- Описано решение через Error Handler вместо Middleware
- Добавлены примеры и объяснение почему middleware не работает

### 2025-10-09
- Переписан документ с фокусом на проблему Progress виджета
- Добавлено подробное описание подхода 9 (динамическое окно)
- Добавлены примеры из реального кода проекта
- Убраны старые разделы про BgManager (оставлена только актуальная информация)

### 2025-10-08
- Добавлен раздел "Альтернативный подход: Прямое использование Bot API"
- Описаны проблемы с BgManager

### 2025-01-08
- Первая версия документации
