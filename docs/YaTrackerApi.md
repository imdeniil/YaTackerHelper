...
### 7. Формат полей пользователей: id вместо login

**Особенность:** API возвращает объекты пользователей только с полем `id`, поле `login` отсутствует.

**Структура из API:**
```json
{
  "lead": {
    "self": "https://api.tracker.yandex.net/v3/users/1130000033442515",
    "id": "1130000033442515",
    "display": "Наталья БМТ Балакина",
    "cloudUid": "ajegkrjakip0ojsn72lf",
    "passportUid": 1130000033442515
  }
}
```
...
lead_id = lead.get("login") or lead.get("id")  # ❌ login не существует!
```
...
if isinstance(lead, dict):
    lead_id = lead.get("id")  # ✅ Всегда используйте id
else:
    lead_id = lead
```
...
- `clients` - заказчики
- `assignee` - исполнитель задачи
- `author` / `createdBy` - автор

### 8. Вложенная структура ParentEntity

**Особенность:** Поле `parentEntity` имеет вложенную структуру с `primary` и `secondary`.

**Структура из API (GET):**
```json
{
  "parentEntity": {
    "primary": {
      "self": "https://api.tracker.yandex.net/v3/entities/portfolio/65cde69d...",
      "id": "65cde69d486b9524503455b7",
      "display": "BrandmasterTeam"
    },
    "secondary": []
  }
}
...
**Формат для API (CREATE):**
```json
{
  "parent_entity": {
    "primary": "65cde69d486b9524503455b7",
    "secondary": []
  }
}
```
...
**Неправильно:**
```python
parent = project.get("parentEntity")
project_data["parent_entity"] = parent.get("id")  # ❌ Строка, а API требует словарь!
```
...
if isinstance(parent, dict):
    ...
    if "secondary" in parent:
        secondary_ids = parent.get("secondary", [])
...
### 9. Расположение полей: fields vs корень объекта

**Особенность:** Некоторые поля могут находиться как в корне объекта, так и в объекте `fields`.

**Структура из API:**
```json
{
  "id": "66c89bca16e2112028b64e92",
  "shortId": 175,
  "entityType": "project",
  "fields": {
...
**Неправильно:**
```python
lead = project.get("lead")  # ❌ Может быть None, если в fields!
```
...
# Проверяем оба места
lead = project.get("lead")
if not lead and "fields" in project:
    lead = project["fields"].get("lead")
```
...
```python
def get_field_value(entity: dict, field_name: str):
    """Извлечь значение поля из сущности (проверяет корень и fields)."""
    value = entity.get(field_name)
    if value is None and "fields" in entity:
        value = entity["fields"].get(field_name)
    return value

# Использование
lead = get_field_value(project, "lead")
team_users = get_field_value(project, "teamUsers")
```

### 10. Naming convention: snake_case vs camelCase

**Особенность:** Библиотека использует `snake_case` для параметров, API возвращает `camelCase`.

**Для создания (create):**
```python
# snake_case в параметрах Python
project = await client.entities.create(
    entity_type="project",
    summary="Новый проект",
    team_users=["user1", "user2"],      # snake_case
    team_access=False,                   # snake_case
    parent_entity={"primary": "id"}      # snake_case
)
```
...
| Python параметр | API response | API create/update |
|----------------|--------------|-------------------|
| `team_users` | `teamUsers` | `team_users` |
| `team_access` | `teamAccess` | `team_access` |
| `parent_entity` | `parentEntity` | `parent_entity` |
| `entity_type` | `entityType` | `entity_type` |
| `short_id` | `shortId` | - |
| `created_by` | `createdBy` | - |

### 11. Полное копирование проекта со всеми полями

**Пример:** Корректное извлечение и копирование всех полей проекта.

```python
async def copy_project(original_project: dict, new_name: str):
    """Создать копию проекта со всеми полями."""

    # Функция для извлечения полей
    def get_field(field_name: str):
        value = original_project.get(field_name)
        if value is None and "fields" in original_project:
            value = original_project["fields"].get(field_name)
        return value
    ...
    # TeamUsers (извлекаем id каждого пользователя)
    team_users = get_field("teamUsers")
    if team_users:
        user_ids = []
        ...
        if user_ids:
            project_data["team_users"] = user_ids

    # TeamAccess
    team_access = get_field("teamAccess")
    if team_access is not None:
        project_data["team_access"] = team_access

    # ParentEntity (преобразуем в нужный формат)
    parent = get_field("parentEntity")
    if parent:
        if isinstance(parent, dict):
            parent_id = None
            secondary_ids = []
    ...
    # Создаем проект
    return await client.entities.create(**project_data)
```
